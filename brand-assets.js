(() => {
  ...
  if (!downloads || document.querySelector(".brand-assets-section, .press-brand-assets")) {
    return;
  }
  ...
  section.className = "press-brand-assets";
  section.id = "brand-assets";
  section.setAttribute("aria-labelledby", "brand-assets-title");
  section.innerHTML = `
    <h2 class="sr-only" id="brand-assets-title" data-brand-title></h2>
  ...
  downloads.insertAdjacentElement("beforebegin", section);
  ...
  const updateCopy = () => {
    ...
    section.querySelector("[data-brand-title]").textContent = text.carousel;
